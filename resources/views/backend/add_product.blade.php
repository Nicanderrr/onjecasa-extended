...
@section('title', 'Add Website Product - ONJECASA')
@section('page-icon', 'bi bi-plus-circle')
@section('page-eyebrow', 'Storefront Catalog')
@section('page-title', 'Add Website Product')
@section('page-description', 'Create a public storefront product and optionally publish it into POS at the same time.')
...
@php
  ...
  $optionPrices = old('option_prices', []);
...
<style>
  ...
  .gallery-preview-option:has(input:checked) {
...
<div class="entity-page website-product-form">
  <section class="entity-hero">
    <div class="entity-hero-copy">
      <p class="eyebrow">New storefront item</p>
      <h1>Add Website Product</h1>
      <p>This creates the product customers see online. Keep the POS checkbox on to make it available in-store too.</p>
    </div>
  ...
  <div class="card shadow entity-card">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('store-product') }}" enctype="multipart/form-data" class="compact-form">
        @csrf
              ...
              <div class="form-group">
                ...
                <select name="description" class="form-control" required>
                  ...
                  @foreach($categories ?? [] as $category)
                    <option value="{{ $category->name }}" @selected(old('description') === $category->name)>{{ $category->name }}</option>
                  @endforeach
                </select>
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" name="price" class="form-control" value="{{ old('price') }}" placeholder="0.00" required>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label>Product Stock</label>
                    <input type="number" min="0" step="1" name="stock" class="form-control" value="{{ old('stock', 0) }}" placeholder="0" required>
          ...
          <div class="product-basic-panel">
            ...
            <div class="form-group">
              ...
              <div class="website-image-upload">
                <img id="showImage" src="{{ asset('upload/no_image.jpg') }}" alt="Product preview">
                <div>
                  <label for="image" class="btn btn-light btn-sm mb-2">
                    <i class="bi bi-upload"></i> Choose Images
                  </label>
                  <input id="image" name="gallery_images[]" type="file" accept="image/*" multiple required>
                  <div id="imageFileName" class="form-hint">No image selected yet</div>
                  <div class="form-hint">Select one or more images. The first image is the thumbnail by default.</div>
                </div>
              ...
              <div class="website-image-upload">
                <div>
                  <label for="productVideos" class="btn btn-light btn-sm mb-2">
                    <i class="bi bi-upload"></i> Choose Videos
                  </label>
                  <input id="productVideos" name="product_videos[]" type="file" accept="video/mp4,video/webm,video/quicktime,video/x-m4v" multiple>
                  <div id="videoFileName" class="form-hint">No video selected yet</div>
                  <div class="form-hint">MP4, WEBM, MOV, or M4V. Keep them short. Max 50MB each.</div>
                </div>
        ...
        <div class="form-group">
          ...
          <textarea rows="4" name="contents" class="form-control" placeholder="Describe the product, option, add-on, availability, and delivery details." required>{{ old('contents') }}</textarea>
        </div>
        ...
        <div class="sync-panel mb-3">
          ...
          <div>
            <strong>Also show this product in POS</strong>
            <small>Auto-checked. When saved, a linked POS product is created for in-store selling.</small>
          </div>
        </div>
...
<script>
  ...
  function renderImagePreviews() {
    ...
    document.getElementById('imageFileName').textContent = selectedImages.length ? selectedImages.length + ' image(s) selected' : 'No image selected yet';
    syncFileInput(input, selectedImages);

    if (!selectedImages.length) {
      document.getElementById('showImage').src = '{{ asset('upload/no_image.jpg') }}';
      return;
    }
    ...
    selectedImages.forEach(function(selectedFile, index) {
      ...
      reader.onload = function(loadEvent) {
        ...
        var item = makeThumbnailOption({
          ...
          inputName: 'thumbnail_image_index',
          value: index,
          checked: index === 0 && !document.querySelector('input[name="thumbnail_video_index"]:checked'),
          label: index === 0 ? 'Thumbnail' : 'Use as thumbnail'
        });
  ...
  function renderVideoPreviews() {
    ...
    previewGrid.innerHTML = '';
    document.getElementById('videoFileName').textContent = selectedVideos.length ? selectedVideos.length + ' video(s) selected' : 'No video selected yet';
    ...
    selectedVideos.forEach(function(selectedFile, index) {
      ...
      item.querySelector('input').addEventListener('change', function() {
        document.querySelectorAll('input[name="thumbnail_image_index"]').forEach(function(imageThumb) { imageThumb.checked = false; });
      });
      previewGrid.appendChild(item);
    });
  }
