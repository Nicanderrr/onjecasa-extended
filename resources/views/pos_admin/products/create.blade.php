...
@section('title', 'Add POS Product - ONJECASA POS')
@section('page-eyebrow', 'In-Store Inventory')
@section('page-title', 'Add POS Product')
@section('page-description', 'Create an in-store product with stock, price, image, and optional storefront sync.')
...
<div class="entity-page">
  <section class="entity-hero">
    <div class="entity-hero-copy">
      <p class="eyebrow">New in-store item</p>
      <h1>Add POS Product</h1>
      <p>This creates a product for cashier sales. Keep website sync checked if the same item should appear online.</p>
    </div>
    <div class="entity-chip">
      <i class="fas fa-bolt"></i>
      SKU can auto-generate
    </div>
  </section>

  <form method="POST" action="{{ route('pos.admin.products.store') }}" enctype="multipart/form-data" class="pos-product-form">
    @csrf
      ...
      <div class="pos-product-fields">
        <section class="pos-product-panel">
          ...
          <div class="form-group mb-0">
            ...
            <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="e.g. Smirnoff Vodka" required>
          </div>
        </section>
        ...
        <section class="pos-product-panel">
          <div class="pos-panel-heading">
            <div>
              <strong>SKU</strong>
              <span>Optional stock code for faster lookup.</span>
            </div>
          ...
          <div class="form-group mb-0">
            ...
            <div class="barcode-input-action">
              <input id="productBarcodeInput" type="text" name="code" value="{{ old('code') }}" class="form-control" inputmode="numeric" autocomplete="off" placeholder="Scan barcode or enter SKU" data-hardware-barcode-capture>
        ...
        <section class="pos-product-panel">
          ...
          <div class="form-group mb-0">
            <label>Stock</label>
            <input type="number" min="0" step="1" name="stock" value="{{ old('stock', 0) }}" class="form-control" placeholder="0" required>
          </div>
        </section>
        ...
        <section class="pos-product-panel pos-product-panel-full">
          ...
          <div class="form-group mb-0">
            ...
            <textarea rows="4" name="description" class="form-control" placeholder="Bottle size, brand notes, pack details, or stock notes.">{{ old('description') }}</textarea>
          </div>
        </section>
      </div>
      ...
      <aside class="pos-product-panel">
        <div class="pos-panel-heading">
          <div>
            ...
            <span>Used in POS product lists and synced website products.</span>
          </div>
          <span class="pos-panel-icon"><i class="fas fa-image"></i></span>
        </div>

        <div class="pos-image-upload">
          <img id="posImagePreview" class="pos-image-preview" src="{{ asset('upload/no_image.jpg') }}" alt="Product image preview">
          <div>
            ...
            <div id="posImageFileName" class="form-hint">No image selected yet. Maximum image size: 5 MB.</div>
          </div>
        </div>
      </aside>
    </div>
    ...
    <label class="pos-sync-panel">
      ...
      <span>
        <strong>Also show this product on website</strong>
        <small>Auto-checked. Saving creates a linked storefront product using this price, stock, description, and image.</small>
      </span>
    </label>
...
<script>
  document.getElementById('posProductImage')?.addEventListener('change', function(event) {
    ...
    if (!file) {
      preview.src = '{{ asset('upload/no_image.jpg') }}';
      fileName.textContent = 'No image selected yet. Maximum image size: 5 MB.';
      return;
    }
